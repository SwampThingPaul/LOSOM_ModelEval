...
# -------------------------------------------------------------------------
file.list=list.files(paste0(data.path,"Iteration_3_Tech_Discussion"),full.names=T,all.files=T,recursive=T)

vals=grep("/Lake_Okeechobee/lo_extreme_hi_lo.pdf",file.list)
file.list[vals]
...
sle.env=data.frame()
for(i in 1:length(NE.data.path)){
  tmp=read.table(paste0(NE.data.path[i],"/sle_flow_envelope.txt"),skip=13)
  PM.val=substring(strsplit(NE.data.path[i],"\\/")[[1]][7],1,1)
  tmp$PM.grp=paste0("PM_",PM.val)
  sle.env=rbind(tmp,sle.env)
}
vals=c("Alt","low","opt","stress.basin","stress.LOK","dam.basin","dam.LOK","high1","high2","PM.grp")
colnames(sle.env)<-vals
sle.env
subset(sle.env,Alt=="CCsimp4")
sle.env$Alt=with(sle.env,ifelse(Alt=="CCsimp4"&PM.grp%in%c("PM_3","PM_4","PM_5"),"CCsimp4(2)",Alt))
subset(sle.env,Alt=="CCsimp4")
...
sle.env=sle.env[duplicated(sle.env[,1:9])==F,]
# write.csv(cre.env[,1:12],paste0(export.path,"20210920_Iter3Opt_CREEnv.csv"),row.names = F)
# PMgrps=cre.env[,c("Alt","PM.grp")]
# nrow(PMgrps)

## CCsimp4 == simp4
subset(cre.env,Alt%in%c("CCsimp4","simp4"))
subset(cre.env,Alt%in%c("CCsimp4(2)","simp4s1"))
...
## 
sle.env2=subset(sle.env,Alt%in%PMgrps.all)
sle.env2=sle.env2[match(PMgrps.all,sle.env2$Alt),]

vars=c("low","opt","stress.LOK","dam.LOK","high2")
tmp.FWO=data.frame()
var.names=paste0("PerFWO.",vars)
for(k in 1:length(vars)){
  tmp.FWO=data.frame(val=((sle.env2[,vars[k]]-sle.env2[1,vars[k]])/sle.env2[1,vars[k]])*100)
  colnames(tmp.FWO)=var.names[k]
  sle.env2=cbind(sle.env2,tmp.FWO)
}

tmp.CC=data.frame()
var.names=paste0("PerCC.",vars)
for(k in 1:length(vars)){
  tmp.CC=data.frame(val=((sle.env2[,vars[k]]-sle.env2[2,vars[k]])/sle.env2[2,vars[k]])*100)
  tmp.CC[1,]=NA
  colnames(tmp.CC)=var.names[k]
  sle.env2=cbind(sle.env2,tmp.CC)
}
...
layout(matrix(1:15,3,5,byrow=T))
  ...
  for(j in 1:length(vars)){
    ...
    if(j==1){mtext(side=3,adj=0,line=-1.25," CRE")}
...
for(i in 1:9){
  alts.val=PMgrps[[i]]
  cols=c("grey20",wesanderson::wes_palette("Zissou1",length(alts.val)-1,"continuous"))
  tmp=subset(sle.env2,Alt%in%alts.val)
  
  tmp=tmp[match(alts.val,tmp$Alt),]
  
  
  png(filename=paste0(plot.path,"Iteration3_TechEval/SLE_RECOVER_SalEnv_",PMval[i],".png"),width=8,height=5,units="in",res=200,type="windows",bg="white")
  par(family="serif",mar=c(1,2,0.5,1),oma=c(4,3.5,2,1),lwd=0.5);
  layout(matrix(1:15,3,5,byrow=T))
  
  ymax=c(300,1500,250,200,250)
  yval=ymax/2
  for(j in 1:length(vars)){
    ylim.val=c(0,ymax[j]);ymaj=seq(ylim.val[1],ylim.val[2],yval[j]);ymin=seq(ylim.val[1],ylim.val[2],yval[j]/2)
    x=barplot(tmp[,vars[j]],col=adjustcolor(cols,0.5),ylim=ylim.val,space=0,axes=F,ann=F)
    ...
    axis_fun(1,x,x,NA,las=2,cex=0.85)
    box(lwd=1)
    mtext(side=3,adj=0,SLE.labs[j],cex=0.7)
    if(i==1){
      text(x,tmp[,vars[j]],round(tmp[,vars[j]],0),font=2,col="black",cex=0.7,pos=1,srt=90,adj=0.5)
    }else{text(x,tmp[,vars[j]],round(tmp[,vars[j]],0),font=2,col="black",pos=ifelse(min(tmp[,vars[j]])<20,3,1),cex=0.7,offset=0.25)}
    if(j==1){mtext(side=3,adj=0,line=-1.25," SLE")}
    if(j==1){mtext(side=2,line=2.5,"Count of\n14-Day Periods",cex=0.75)}
    if(j==5){mtext(side=3,adj=1,PMval[i])}
  }
  ...
  ymax.vals=list(
    ...
    c(100,100,110,100,100),
    c(100,100,110,100,100),
    c(110,100,110,100,100),
    c(100,100,110,100,100)
  )
  ymax=ymax.vals[[i]]# c(20,20,1100,1000,50)
  yval=ymax/2
  
  # ylim.val=c(-120,100);by.y=50;ymaj=seq(max(c(-100,ylim.val[1])),ylim.val[2],by.y);ymin=seq(max(c(-100,ylim.val[1])),ylim.val[2],by.y/2)
  for(j in 1:length(vars)){
    ylim.val=c(-ymax[j],ymax[j]);ymaj=seq(ylim.val[1],ylim.val[2],yval[j]);ymin=seq(ylim.val[1],ylim.val[2],yval[j]/2)
    x=barplot(tmp[,var.FWO[j]],col=adjustcolor(cols,0.5),ylim=ylim.val,space=0,axes=F,ann=F)
    abline(h=0)
    axis_fun(2,ymaj,ymin,ymaj)
    axis_fun(1,x,x,NA,las=2,cex=0.85)
...
for(i in 1:9){
  ...
  png(filename=paste0(plot.path,"Iteration3_TechEval/SLE_RegQ_",PMval[i],".png"),width=4,height=5,units="in",res=200,type="windows",bg="white")
  par(family="serif",mar=c(1,2,0.5,1),oma=c(4,3.5,2,1),lwd=0.5);
  layout(matrix(1:3,3,1,byrow=T))
  
  ylim.val=c(0,250);by.y=100;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
  x=barplot(tmp[,"SLE"],col=adjustcolor(cols,0.5),ylim=ylim.val,space=0,axes=F,ann=F)
  axis_fun(2,ymaj,ymin,ymaj)
  axis_fun(1,x,x,NA,las=2,cex=0.85)
  box(lwd=1)
  text(x,tmp[,"SLE"],round(tmp[,"SLE"],0),font=2,col="black",pos=1,cex=0.7,offset=0.25)
  mtext(side=3,adj=0,"Mean Annual Flood Control Releases",cex=0.8,col="grey40")
  mtext(side=3,adj=0,line=-1.25," SLE")
  mtext(side=2,line=2.5,"Discharge Volume\n(x1000 Ac-Ft Y\u207B\u00B9)",cex=0.75)
  mtext(side=3,adj=1,PMval[i],cex=0.75)
  
  ylim.val=c(-100,100);by.y=50;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
  x=barplot(tmp[,"PerFWO.SLE"],col=adjustcolor(cols,0.5),ylim=ylim.val,space=0,axes=F,ann=F)
  abline(h=0)
  axis_fun(2,ymaj,ymin,ymaj)
  axis_fun(1,x,x,NA,las=2,cex=0.85)
  box(lwd=1)
  text(x,tmp[,"PerFWO.SLE"],round(tmp[,"PerFWO.SLE"],0),font=2,col="black",pos=ifelse(tmp[,"PerFWO.SLE"]<0,1,3),cex=0.75,offset=0.1)
  mtext(side=2,line=2.5,"Average Percent\nDifference to FWO",cex=0.75)
  
  ylim.val=c(-50,300);by.y=100;ymaj=seq(min(c(0,ylim.val[1])),ylim.val[2],by.y);ymin=seq(min(c(0,ylim.val[1])),ylim.val[2],by.y/2)
  x=barplot(tmp[,"PerCC.SLE"],col=adjustcolor(cols,0.5),ylim=ylim.val,space=0,axes=F,ann=F)
  abline(h=0)
  axis_fun(2,ymaj,ymin,ymaj)
  axis_fun(1,x,x,alts.val,las=2,cex=0.85)
  box(lwd=1)
  text(x,tmp[,"PerCC.SLE"],round(tmp[,"PerCC.SLE"],0),font=2,col="black",pos=ifelse(tmp[,"PerCC.SLE"]<0|is.na(tmp[,"PerCC.SLE"])==T,1,3),cex=0.75,offset=0.1)
  mtext(side=2,line=2.5,"Average Percent\nDifference to CC",cex=0.75)
...
# LOK Stage Metrics -------------------------------------------------------
lok.stg=read.csv(paste0(data.path,"Iteration_3_Tech_Discussion/lok_stg_data.csv"))
head(lok.stg)

subset(lok.stg,Alt=="CCsimp4")
lok.stg$Alt=with(lok.stg,ifelse(Alt=="CCsimp4"&PM.grp%in%c("PM_3","PM_4","PM_5"),"CCsimp4(2)",Alt))
subset(lok.stg,Alt=="CCsimp4")

lok.stg=lok.stg[duplicated(lok.stg[,c(1,4:6)])==F,];# c(1,4:6) due to missing data
nrow(lok.stg)
lok.reg=lok.reg[match(PMgrps.all,lok.reg$Alt),]
