...
# png(filename=paste0(plot.path,"Iteration_1/LO_totalDays.png"),width=5,height=6,units="in",res=200,type="windows",bg="white")
...
# png(filename=paste0(plot.path,"Iteration_1/LO_iter1_freq_bxp.png"),width=6.5,height=5,units="in",res=200,type="windows",bg="white")
...
# png(filename=paste0(plot.path,"Iteration_1/LO_StageDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/LO_StageDuration.tiff"),width=8,height=4,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
for(i in 3:n.alts){
  ...
  if(i%in%c(3,7,11,15)){axis_fun(2,ymaj,ymin,ymaj)}else{axis_fun(2,ymaj,ymin,NA)}
  box(lwd=1)
}
...
# write.csv(highlow.ecdf,paste0(export.path,"highlow_ecdf_rslt.csv"),row.names = F)
...
# png(filename=paste0(plot.path,"Iteration_1/LakeO_EnvScore_bxp.png"),width=6.5,height=5,units="in",res=200,type="windows",bg="white")
...
# png(filename=paste0(plot.path,"Iteration_1/LakeO_EnvScore_sum.png"),width=4,height=5,units="in",res=200,type="windows",bg="white")
...
plot(norm.lakeO.stage.scr.WY.sum$FWO.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(norm.lakeO.stage.scr.WY.sum,segments(FWO.diff,1:15,rep(0,15),1:15))
with(norm.lakeO.stage.scr.WY.sum,points(FWO.diff,1:15,pch=21,bg="dodgerblue1",lwd=0.1))
axis_fun(2,1:15,1:15,norm.lakeO.stage.scr.WY.sum$Alt)
...
plot(rec.lakeO.stage.scr.WY.sum$FWO.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(rec.lakeO.stage.scr.WY.sum,segments(FWO.diff,1:15,rep(0,15),1:15))
with(rec.lakeO.stage.scr.WY.sum,points(FWO.diff,1:15,pch=21,bg="indianred1",lwd=0.1))
...
q.dat$bloom.period=with(q.dat,ifelse(as.numeric(format(Date,"%m"))%in%c(7:9),"bloom","no.bloom"))

# CRE ---------------------------------------------------------------------
# Flow duration curves
# png(filename=paste0(plot.path,"Iteration_1/S79_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S79_FlowDuration.tiff"),width=8,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/S77_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S77_FlowDuration.tiff"),width=8,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
opt.AUC=data.frame()
for(i in 1:n.alts){
  tmp=ecdf_fun(subset(q.dat,SITE=="S79"&Alt==alts.sort[i])$FLOW)
  # tmp2=with(subset(tmp,value<=2100&value>=750),spline(x=1-proportion,y=value))
  rslt=with(subset(tmp,value<=2100&value>=750),DescTools::AUC(x=1-proportion,y=value))
  # rslt=with(subset(tmp,value<=2100&value>=750),MESS::auc(x=1-proportion,y=value,type="spline"))
  opt.AUC=rbind(data.frame(Alt=alts.sort[i],AUC=rslt),opt.AUC)
}
opt.AUC

x=barplot(opt.AUC$AUC)
...
# ggsave(CRE.flow.tern,filename=paste0(plot.path,"Iteration_1/CRE_FlowCat.png"),width=8,heigh=7.5,unit="in",device="png")

# png(filename=paste0(plot.path,"Iteration_1/CRE_iter1_freq_bxp.png"),width=6.5,height=5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/CRE_iter1_freq_bxp.tiff"),width=5.5,height=7,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
# png(filename=paste0(plot.path,"Iteration_1/S79_PercentDryQ_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S79_PercentDryQ_bxp.tiff"),width=6,height=4,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
# png(filename=paste0(plot.path,"Iteration_1/S79_PercentDryQ.png"),width=8,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S79_PercentDryQ.tiff"),width=8,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))

xlim.val=c(1966,2016);by.x=20;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/by.x)
ylim.val=c(0,105);by.y=20;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)

for(i in 3:n.alts){
  plot(0:1,0:1,ylim=ylim.val,xlim=xlim.val,ann=F,axes=F,type="n")
  abline(h=ymaj,v=xmaj,lty=3,col="grey")
  with(subset(cre.q.season,Alt==alts.sort[1]),lines(WY,PerDry,col=cols[1],lty=1,lwd=1.5))
  with(subset(cre.q.season,Alt==alts.sort[2]),lines(WY,PerDry,col=cols[2],lty=2,lwd=1))
  with(subset(cre.q.season,Alt==alts.sort[i]),lines(WY,PerDry,col=adjustcolor(cols[i],0.5),lwd=1.5))
  legend("topright",legend=alts.sort[c(1,2,i)],
         lty=c(1,2,1),lwd=c(1.5,1.5,1.5),col=c(cols[1:2],adjustcolor(as.character(cols[i]),0.5)),
         ncol=1,cex=0.8,bty="n",y.intersp=1,x.intersp=0.75,xpd=NA,xjust=0.5,yjust=0.5)
  if(i%in%c(3:11)){axis_fun(1,xmaj,xmin,NA)}else{axis_fun(1,xmaj,xmin,format(xmaj))}
...
mtext(side=2,line=2.5,outer=T,"Percent Dry Season Discharge")
...
# png(filename=paste0(plot.path,"Iteration_1/S79_PercentBasinQ_bxp.png"),width=6.5,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S79_PercentBasinQ_bxp.tiff"),width=5.5,height=6.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
...
# Bloom period ------------------------------------------------------------
bloom.q.season=cast(subset(q.dat,SITE%in%c("S79")),WY+Alt~bloom.period,value="FLOW",sum,na.rm=T)
...
bloom.q.season.WY.sum=ddply(bloom.q.season,"Alt",summarise,mean_bloom=mean(per_bloom,na.rm=T))
bloom.q.season.WY.sum=bloom.q.season.WY.sum[match(alts.sort,bloom.q.season.WY.sum$Alt),]

# png(filename=paste0(plot.path,"Iteration_1/S79_PercentBloomQ_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S79_PercentBloomQ_bxp.tiff"),width=6,height=4,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));

ylim.val=c(0,100);by.y=20;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
boxplot(per_bloom~Alt,bloom.q.season,ylim=ylim.val,axes=F,ann=F,outline=F,col=cols)
points(1:n.alts,bloom.q.season.WY.sum$mean_bloom,pch=21,bg="springgreen",lwd=0.1,cex=1.25)
abline(h=median(subset(bloom.q.season,Alt==alts.sort[1])$per_bloom),lty=2,col="black")
abline(h=subset(bloom.q.season.WY.sum,Alt==alts.sort[1])$mean_bloom,lty=2,col="springgreen")
axis_fun(2,ymaj,ymin,ymaj)
axis_fun(1,1:n.alts,1:n.alts,alts.sort,las=2)
abline(v=2.5)
box(lwd=1)
...
# png(filename=paste0(plot.path,"Iteration_1/S79_WYQ_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
...
# SLE ---------------------------------------------------------------------
# Flow duration curves
# png(filename=paste0(plot.path,"Iteration_1/S80_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S80_FlowDuration.tiff"),width=8,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/S308_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
# tiff(filename=paste0(plot.path,"Iteration_1/tiff/S308_FlowDuration.tiff"),width=8,height=3.5,units="in",res=200,type="windows",compression=c("lzw"),bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# ggsave(SLE.flow.tern,filename=paste0(plot.path,"Iteration_1/SLE_FlowCat.png"), width=8,heigh=7.5,unit="in",device="png")


# png(filename=paste0(plot.path,"Iteration_1/SLE_iter1_freq_bxp.png"),width=6.5,height=5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,3,0.25,1),oma=c(4,3,1,0.25));
layout(matrix(1:3,3,1,byrow=T))
...
sle.q.season.sum=ddply(sle.q.season,"Alt",summarise,mean.val=mean(PerDry,na.rm=T))
sle.q.season.sum=sle.q.season.sum[match(alts.sort,sle.q.season.sum$Alt),]

# png(filename=paste0(plot.path,"Iteration_1/S80_PercentDryQ_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
ylim.val=c(0,100);by.y=20;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
boxplot(PerDry~Alt,sle.q.season,ylim=ylim.val,axes=F,ann=F,outline=F,col=cols)
points(1:n.alts,sle.q.season.sum$mean,pch=21,bg="springgreen",lwd=0.1,cex=1.25)
...
# png(filename=paste0(plot.path,"Iteration_1/S80_PercentBasinQ_bxp.png"),width=6.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(4,3.75,1,0.25));
layout(matrix(1:2,2,1,byrow=T))
ylim.val=c(0,100);by.y=20;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
boxplot(PerBasin~Alt,basin.q.season.WY,ylim=ylim.val,axes=F,ann=F,outline=F,col=cols)
...
# Bloom period ------------------------------------------------------------
bloom.q.season=cast(subset(q.dat,SITE%in%c("S80")),WY+Alt~bloom.period,value="FLOW",sum,na.rm=T)
bloom.q.season$TFlow=rowSums(bloom.q.season[,c("bloom","no.bloom")],na.rm=T)
bloom.q.season$per_bloom=with(bloom.q.season,(bloom/TFlow)*100)
bloom.q.season$Alt=factor(bloom.q.season$Alt,levels=alts.sort)
...
# png(filename=paste0(plot.path,"Iteration_1/S80_PercentBloomQ_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
...
# png(filename=paste0(plot.path,"Iteration_1/S80_WYQ_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));

ylim.val=c(0,1.7e6);by.y=0.5e6;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
...
# Flow duration curves
# png(filename=paste0(plot.path,"Iteration_1/S351_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/S352_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/S354_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/EAA_FlowDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/EAA_discharge_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
...
# png(filename=paste0(plot.path,"Iteration_1/EAA_discharge_sum.png"),width=3,height=5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.7,0.25,1),oma=c(2,5,1,0.25));

xlim.val=c(-25,50);by.x=25;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(EAA.q.WY.sum$FWO.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(EAA.q.WY.sum,segments(FWO.diff,1:15,rep(0,15),1:15))
with(EAA.q.WY.sum,points(FWO.diff,1:15,pch=21,bg="dodgerblue1",lwd=0.1))
axis_fun(2,1:15,1:15,EAA.q.WY.sum$Alt)
axis_fun(1,xmaj,xmin,xmaj,line=-0.7,cex=0.8);box(lwd=1)
...
# png(filename=paste0(plot.path,"Iteration_1/WCA3_6263_StageDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/WCA3_636465_StageDuration.png"),width=7.5,height=5.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
layout(matrix(1:16,4,4,byrow=T))
...
# png(filename=paste0(plot.path,"Iteration_1/WCA_6263_perdiff.png"),width=4.5,height=5.25,units="in",res=200,type="windows",bg="white")
...
plot(HiLo.close.sum$FWO.Lo.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(HiLo.close.sum,segments(FWO.Lo.diff,1:15,rep(0,15),1:15))
with(HiLo.close.sum,points(FWO.Lo.diff,1:15,pch=21,bg="dodgerblue1",lwd=0.1))
axis_fun(2,1:15,1:15,HiLo.close.sum$Alt)
axis_fun(1,xmaj,xmin,xmaj,line=-0.7,cex=0.8);box(lwd=1)
...
plot(HiLo.close.sum$FWO.Hi.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(HiLo.close.sum,segments(FWO.Hi.diff,1:15,rep(0,15),1:15))
with(HiLo.close.sum,points(FWO.Hi.diff,1:15,pch=21,bg="indianred1",lwd=0.1))
axis_fun(2,1:15,1:15,NA)
axis_fun(1,xmaj,xmin,xmaj,line=-0.7,cex=0.8);box(lwd=1)
...
# png(filename=paste0(plot.path,"Iteration_1/WCA_636465_perdiff.png"),width=3,height=5.25,units="in",res=200,type="windows",bg="white")
...
plot(Low.wca3.sum$FWO.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(Low.wca3.sum,segments(FWO.diff,1:15,rep(0,15),1:15))
with(Low.wca3.sum,points(FWO.diff,1:15,pch=21,bg="dodgerblue1",lwd=0.1))
axis_fun(2,1:15,1:15,Low.wca3.sum$Alt)
axis_fun(1,xmaj,xmin,xmaj,line=-0.7,cex=0.8);box(lwd=1)
...
# png(filename=paste0(plot.path,"Iteration_1/WCA_S333HW_perdiff.png"),width=3,height=5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1.5,0.7,0.5,1),oma=c(1.5,5,1,0.25));

xlim.val=c(-30,2);by.x=10;xmaj=c(0,seq(xlim.val[1],xlim.val[2],by.x));xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(S333HW.sum$FWO.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(S333HW.sum,segments(FWO.diff,1:15,rep(0,15),1:15))
with(S333HW.sum,points(FWO.diff,1:15,pch=21,bg="dodgerblue1",lwd=0.1))
axis_fun(2,1:15,1:15,S333HW.sum$Alt)
axis_fun(1,xmaj,xmin,xmaj,line=-0.7,cex=0.8);box(lwd=1)
...
plot(mean_636465~Date,subset(wca.stg.dat.xtab,Alt=="LSM25B"),type="l")
par(new=T);plot(FWM~Date,subset(S12DS333.TP.FWM,Alt=="LSM25B"),type="l",col="red",yaxt="n");axis(4)
...
plot(FWM~FedWY,subset(WY.FWM,Alt=="ECRE"))
with(subset(WY.FWM,Alt=="LSM25B"),points(FedWY,FWM,pch=21,bg="red"))
...
# png(filename=paste0(plot.path,"Iteration_1/SRS_FWM_bxp.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2.75,0.75,0.25,1),oma=c(2.5,3.75,1,0.25));
...
abline(v=2.5)
box(lwd=1)
mtext(side=3, adj=0,"SRS (S12s+[(S333+S355AB+S356)-S334])")
mtext(side=2,line=2.25,"TP FWM (\u03BCg L\u207B\u00B9)")
mtext(side=1,line=4,"Model Alternatives")
dev.off()

# png(filename=paste0(plot.path,"Iteration_1/SRS_FWM_perdiff.png"),width=3,height=5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(1.5,0.7,0.5,1),oma=c(1.5,5,1,0.25));

xlim.val=c(-15,2);by.x=5;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2)
plot(WY.FWM.sum$FWO.diff,1:15,ann=F,axes=F,type="n",xlim=xlim.val)
abline(v=0)
with(WY.FWM.sum,segments(FWO.diff,1:15,rep(0,15),1:15))
with(WY.FWM.sum,points(FWO.diff,1:15,pch=21,bg="dodgerblue1",lwd=0.1))
axis_fun(2,1:15,1:15,WY.FWM.sum$Alt)
axis_fun(1,xmaj,xmin,xmaj,line=-0.7,cex=0.8);box(lwd=1)
mtext(side=3,adj=0,"SRS FWM",cex=1,col="black")
...
# png(filename=paste0(plot.path,"Iteration_1/WCA3_WYQ_bxp.png"),width=6.5,height=5.5,units="in",res=200,type="windows",bg="white")
